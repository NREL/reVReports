...
def recast_categories(df, col, lkup, cell_size_sq_km=None):
    ...
    try:
        ...
        raise reVReportsTypeError(msg) from e
    ...
    col_df = col_df.rename(columns=lkup)
...
def unpack_characterizations(in_df, characterization_remapper, cell_size_m=90):
    ...
    This function unpacks characterization data from the input supply
    curve dataframe according to the specifications in
    ``characterization_remapper``, including converting values from
    embedded JSON strings to new standalone columns.
    ...
    characterization_remapper : dict
        ...
        column of ``in_df`` containing characterization JSON data. Only
        the columns you want to unpack need to be included.

        The corresponding value should be a dictionary with the
        following keys: "method", "recast", and "lkup" OR "rename".
        Details for each are provided below:

            - "method": Must be one of "category", "sum", "mean", or
                None. Note: These correspond to the "method" used for
                the corresponding layer in the "data_layers" input
                to reV supply-curve aggregation configuration.
            - "recast": Must be one of "area" or None. This defines how
                values in the JSON will be recast to new columns. If
                "area" is specified, they will be converted to area
                values. If null, they will not be changed and will be
                passed through as-is.
            - "lkup": This is a dictionary for remapping categories to
                new column names (see documentation of
                recast_categories() for more information). It should be
                used when "method" = "category". It can also be
                specified as null to skip unpacking of the column.
            - "rename": This is a string indicating what name to use for
                the new column. This should be used when
                "method" != "category".

        A valid example for this parameter can be loaded from
        ``tests/data/characterization-map.json``.
    cell_size_m : int, default=90
        Optional cell size of the characterization layers used in reV.
        Default is the current standard value of 90m.
    ...
    for char_col, col_remapper in tqdm.tqdm(
        characterization_remapper.items(), desc="Unpacking Characterizations"
    ):
        ...
        if method == "category":
            in_df = _handle_category_recast(
                in_df, char_col, recast, lkup, cell_size_sq_km
            )
        elif method == "sum":
            in_df = _handle_sum_recast(
                in_df, char_col, recast, rename, cell_size_sq_km
            )
        elif method == "mean":
            in_df = _handle_mean_recast(in_df, char_col, recast, rename)
        elif method is None:
...
def validate_characterization_remapper(
    ...
    characterization_remapper : dict
        This dictionary defines how to unpack and recast values from the
        characterization JSON strings to new columns. See documentation
        of unpack_characterizations() for details.
    ...
    for col_name, col_remapper in characterization_remapper.items():
        ...
        _validate_lkup_rename_recast(method, lkup, rename, recast, col_name)
...
def _handle_category_recast(in_df, char_col, recast, lkup, cell_size_sq_km):
    ...
    if recast == "area":
        return recast_categories(in_df, char_col, lkup, cell_size_sq_km)

    if recast is None:
        return recast_categories(in_df, char_col, lkup, None)
...
def _validate_lkup_rename_recast(method, lkup, rename, recast, col_name):
    if method == "category":
        ...
        _validate_lkup_rename_for_sum_mean(method, lkup, rename, col_name)
    elif method is None:
        _validate_lkup_rename_recast_for_no_method(
            lkup, rename, recast, col_name
        )
...
def _validate_lkup_rename_for_category(lkup, rename, col_name):
    if lkup is not None and not isinstance(lkup, dict):
        msg = (
            ...
            "Must be a dict or None when method='category'."
        )
        raise reVReportsValueError(msg)
    if rename is not None:
        msg = (
            f"{col_name} - Invalid value for rename: {rename}."
            "Must be None when method='category'."
        )
        raise reVReportsValueError(msg)
