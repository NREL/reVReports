...
from reVReports.characterizations import (
...
def test_recast_categories_pass_through(data_dir):
    ...
    mock_df = recast_categories(mock_df, col, lkup, cell_size_sq_km)
...
def test_recast_categories_recast_to_area(data_dir):
    ...
    mock_df = recast_categories(mock_df, col, lkup, cell_size_sq_km)
    for k, v in mock_char.items():
        assert np.all(mock_df[f"{lkup[k]}_area_sq_km"] == v * cell_size_sq_km)
